...
bool areAllWritten(Element* arr, int l)
{
...
bool isCurrentNAdded(Element* arr, int l, int n)
{
	for (size_t i = 0; i < l; i++)
	{
		if (arr[i].value == n)
...
int** compositionToTable()
{
	...
	//find length
	std::string p1, p2;
	std::cout << "Enter two permutations Ex:" << "\n";
	std::cout << "(1 5 4 6)(2 3) & (1 2 3)(4 5)" << "\n";
	std::string wholeLine = "(";
	std::cin.ignore();
	std::string getLine;
	std::getline(std::cin, getLine);
	wholeLine = wholeLine + getLine;
	...
	while (!areAllWritten(list, max))
	{
		for (size_t i = 1; i <= max;)
		{
			...
			isThisAdded = isCurrentNAdded(list, max, i);
